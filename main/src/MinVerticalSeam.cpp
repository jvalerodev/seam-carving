...
void write_image(const string &filename, const vector<vector<Color>> &pixels)
{
  ...
  string fn{filename.substr(0, dot_position) + string{"-vertical-seam"} + filename.substr(dot_position)};
...
int main(int argc, char *argv[])
{
  ...
  write_image(argv[1], Seam::min_vertical_seam_to_colors(pixels, computed_seam.first));
